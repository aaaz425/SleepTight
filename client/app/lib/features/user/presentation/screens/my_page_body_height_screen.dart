...
final myPageInfoHeightFormValidProvider = StateProvider<bool>((ref) => false);
// 폼 내용 변경 여부 상태를 위한 StateProvider
final myPageInfoHeightFormHasChangedProvider = StateProvider<bool>(
  (ref) => false,
);

class MyPageBodyHeightScreen extends ConsumerWidget {
  MyPageBodyHeightScreen({super.key});

  final _formKey = GlobalKey<FormBuilderState>();

  // NameFormField 내부의 FormBuilderTextField 이름들 (실제 사용하는 이름으로 변경 필요)
  static const String heightFieldName = 'height';
  static const String lengthUnitFieldName = 'length_unit';

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final user = ref.watch(userModelProvider);
    final isFormValid = ref.watch(myPageInfoHeightFormValidProvider);
    final formHasChanged = ref.watch(myPageInfoHeightFormHasChangedProvider);

    final initialLengthUnit = user?.lengthUnit;

    // FormBuilder에 전달할 초기값 맵 구성
    final initialFormValues = {
      heightFieldName: user?.height.toString() ?? '',
      lengthUnitFieldName: initialLengthUnit ?? 'cm',
    };
      ...
      child: Scaffold(
        appBar: MyPageAppBar(
          onBack: () {
            if (context.canPop()) {
              context.pop();
              ref.invalidate(myPageInfoHeightFormHasChangedProvider);
              ref.invalidate(myPageInfoHeightFormValidProvider);
            }
          },
        ),
        body: FormBuilder(
          ...
          onChanged: () {
            ...
            if (form != null) {
              ...
              ref.read(myPageInfoHeightFormValidProvider.notifier).state =
                  isValid;
              ...
              ref.read(myPageInfoHeightFormHasChangedProvider.notifier).state =
                  changed;
            } else {
              ref.read(myPageInfoHeightFormValidProvider.notifier).state =
                  false;
            ...
            child: CustomButton(
              onPressed: () async {
                ...
                if (form.saveAndValidate()) {
                  ...
                  final String newHeight = values[heightFieldName] as String;
                  final String newLengthUnit =
                      values[lengthUnitFieldName] as String;

                  // 3. API 요청 객체 생성
                  final request = UpdateUserHeightRequest(
                    height: num.parse(newHeight),
